...
> Google Play（Android）のみに絞った最新の手順は
> [google-play-release-guide.md](google-play-release-guide.md)（[HTML版](google-play-release-guide.html)）にまとめています。
> こちらを使う場合、本書のGoogle Play関連セクション（6〜8）は参考程度で構いません。
...
2. 以下の画面を撮影する（最低4〜6枚推奨）
   ...
   - まち画面（発展した地球儀・累積発電量・ストックが表示された状態）
   - 履歴画面
   - 設定画面（体重・速度・まちの名前）
   - （任意）スクリーンショットモードのまち画面
